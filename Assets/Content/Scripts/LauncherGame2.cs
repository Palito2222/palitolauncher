...
public class LauncherGame2 : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        downloadedFilePath = PlayerPrefs.GetString("DownloadedFilePath2");

        // Recuperar el estado del juego descargado al iniciar el launcher
        gameDownloaded = PlayerPrefs.GetInt("GameDownloaded2", 0) == 1;
        ...
        currentTag = PlayerPrefs.GetString("CurrentTag2", currentTag);
        ...
        if (www.result == UnityWebRequest.Result.Success)
        {
            if (updateAvailable)
            {
                ...
                if (!File.Exists(downloadPath))
                {
                    ...
                    PlayerPrefs.SetString("CurrentTag2", currentTag);

                    // Actualizar el estado del juego descargado
                    gameDownloaded = true;
                    updateAvailable = false;
                    PlayerPrefs.SetInt("GameDownloaded2", 1);
        ...
        if (www.result == UnityWebRequest.Result.Success)
        {
            ...
            else
            {
                ...
                if (releaseResponse.assets != null && releaseResponse.assets.Length > 0)
                {
                    ...
                    {
                        string downloadUrl = asset.browser_download_url;
                        string filePath = PlayerPrefs.GetString("DownloadedFilePath2");
    ...
    private void RunGame()
    {
        ...
        else
        {
            ...
            PlayerPrefs.SetInt("GameDownloaded2", 0);
    ...
    public string SelectFolder()
    {
        ...
        if (dlg.ShowDialog(IntPtr.Zero) == true)
        {
            ...
            PlayerPrefs.SetString("DownloadedFilePath2", Path.Combine(downloadedFilePath, "CCTycoon"));
